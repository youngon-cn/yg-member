...
define('navbar', class extends WeElement {
  ...
  installed() {
    document.addEventListener('click', e => {
      if (
        this.navBarOpenId.every(
          ids => ids !== e.target.id && ids !== e.target.parentNode.id
        ) &&
        this.showNavBar
      ) {
  ...
  gotoHere = grade => {
    const { offsetTop = 0 } = document.querySelector(`#grade-${grade}`) || {}
